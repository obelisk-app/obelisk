...
export default function LoginModal({
  ...
  useEffect(() => {
    if (!isOpen) return;
    const check = () => setHasNip07(isNip07Available());
    check();
  ...
  // Generate nostrconnect URI when bunker QR tab is selected
  useEffect(() => {
    ...
    const generate = async () => {
      try {
        ...
        await login(signer);
        await Promise.race([
          authenticateWithBackend(signer),
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('Backend auth timed out')), 60000),
          ),
        ]);
        ...
        setUser({ pubkey, npub: nip19.npubEncode(pubkey) }, 'bunker');
        syncProfile();
  ...
  }, [method, bunkerTab]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleLogin = async (loginMethod: LoginMethod) => {
    ...
    try {
      ...
      switch (loginMethod) {
        case 'extension': {
          if (!window.nostr) throw new Error('Nostr extension not detected. Make sure nostr-wot, Amber, nos2x, or a similar extension is installed.');
        ...
        case 'nsec': {
          ...
          signer = new PrivateKeySigner(hex);
          break;
        }
        case 'bunker': {
      ...
      await Promise.race([
        authenticateWithBackend(signer),
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('Backend auth timed out after 60s')), 60000),
        ),
      ]);

      setUser({ pubkey, npub: nip19.npubEncode(pubkey) }, loginMethod);
      syncProfile();
      if (onSuccess) onSuccess(); else onClose();
    } catch (err) {
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      setUser({ pubkey, npub: nip19.npubEncode(pubkey) }, 'nsec');
    } catch (err) {
      ...
      <div className="bg-lc-dark rounded-2xl max-w-md w-full p-8 border border-lc-border shadow-2xl max-h-[90vh] overflow-y-auto animate-modal-card-in">
        ...
        {showProfileSetup ? (
          ...
          <div className="space-y-5">
            ...
            <div className="p-4 bg-lc-olive/20 border border-lc-green/10 rounded-xl space-y-3">
              ...
              <div className="flex flex-col gap-2">
                <a href="https://chromewebstore.google.com/detail/nostr-wot/gfmefgdkmjpjinecjchlangpamhclhdo" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-lc-green hover:text-lc-green/80 transition">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                  Chrome Web Store
                </a>
                <a href="https://nostr-wot.com/downloads" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm text-lc-muted hover:text-lc-white transition">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
            ...
            <button onClick={handleDownloadNsec} className="w-full lc-pill lc-pill-secondary text-sm flex items-center justify-center gap-2">
            ...
            <label className="flex items-start gap-3 p-3 bg-lc-black/40 border border-lc-border rounded-xl cursor-pointer select-none">
              <input type="checkbox" checked={backupConfirmed} onChange={(e) => setBackupConfirmed(e.target.checked)} className="mt-0.5 w-4 h-4 accent-lc-green flex-shrink-0 cursor-pointer" />
            ...
            <button onClick={() => setShowProfileSetup(true)} disabled={!backupConfirmed} className="w-full lc-pill lc-pill-primary text-sm flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed">
          ...
          <div className="space-y-3">
            ...
            {hasNip07 && (
              <button onClick={() => handleLogin('extension')} disabled={isLoading} className="w-full flex items-center gap-4 p-4 bg-lc-olive/40 hover:bg-lc-olive/60 border border-lc-green/20 rounded-xl transition-all duration-200 disabled:opacity-50 group">
                ...
                <div className="text-left flex-1">
                  ...
                  <div className="text-sm text-lc-muted">{loadingMethod === 'extension' ? 'Connecting...' : 'nostr-wot, Amber, nos2x, or similar'}</div>
                </div>
                {loadingMethod !== 'extension' && (
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a3a3a3" strokeWidth="2" className="opacity-0 group-hover:opacity-100 transition"><polyline points="9 18 15 12 9 6"/></svg>
                )}
              </button>
            )}

            {/* nsec */}
            <button onClick={() => setMethod('nsec')} disabled={isLoading} className="w-full flex items-center gap-4 p-4 bg-lc-card hover:bg-lc-border/50 border border-lc-border rounded-xl transition-all duration-200 disabled:opacity-50 group">
            ...
            {/* Bunker */}
            <button onClick={() => setMethod('bunker')} disabled={isLoading} className="w-full flex items-center gap-4 p-4 bg-lc-card hover:bg-lc-border/50 border border-lc-border rounded-xl transition-all duration-200 disabled:opacity-50 group">
              <div className="w-11 h-11 bg-lc-border rounded-xl flex items-center justify-center group-hover:bg-lc-border/80 transition">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#a3a3a3" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            {/* Create New Account */}
            <button onClick={handleCreateAccount} disabled={isLoading || creatingAccount} className="w-full flex items-center gap-4 p-4 bg-lc-card hover:bg-lc-border/50 border border-lc-border rounded-xl transition-all duration-200 disabled:opacity-50 group">
              ...
              <div className="text-left flex-1">
                ...
                <div className="text-sm text-lc-muted">{creatingAccount ? 'Generating keys...' : 'Generate a fresh Nostr identity'}</div>
              </div>
              {!creatingAccount && (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#a3a3a3" strokeWidth="2" className="opacity-0 group-hover:opacity-100 transition"><polyline points="9 18 15 12 9 6"/></svg>
              )}
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm text-lc-muted mb-2 font-medium">Enter your nsec (private key)</label>
              <input
                type="password"
                value={nsecInput}
              ...
              <p className="mt-2.5 text-xs text-lc-muted flex items-center gap-1.5">
                ...
                Never share your nsec. It will be stored encrypted at rest.
              </p>
            </div>
            <button onClick={() => handleLogin('nsec')} disabled={isLoading || !nsecInput.trim()} className="w-full lc-pill lc-pill-primary text-sm disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
          ...
          <div className="space-y-5">
            ...
            <div className="flex bg-lc-black rounded-xl p-1 border border-lc-border/50">
              <button onClick={() => setBunkerTab('qr')} className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-all ${bunkerTab === 'qr' ? 'bg-lc-border text-lc-white' : 'text-lc-muted hover:text-lc-white'}`}>
                QR Code
              </button>
              <button onClick={() => setBunkerTab('url')} className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-all ${bunkerTab === 'url' ? 'bg-lc-border text-lc-white' : 'text-lc-muted hover:text-lc-white'}`}>
                Bunker URL
              </button>
            </div>
            ...
            {bunkerTab === 'qr' ? (
              <div className="flex flex-col items-center space-y-4">
                {connectUri ? (
                  <>
                    <div className="bg-white p-4 rounded-2xl mb-1">
                      <QRCodeSVG value={connectUri} size={200} level="M" bgColor="#ffffff" fgColor="#0a0a0a" />
                    </div>
                    <p className="text-sm text-lc-muted text-center">Scan with your signer app (Amber, nsec.app, etc.)</p>

                    <div className="flex flex-col gap-3 w-full max-w-[200px]">
                      <button onClick={handleCopyUri} className="flex items-center justify-center gap-2 text-xs text-lc-muted hover:text-lc-green transition px-3 py-2 bg-lc-black rounded-xl border border-lc-border/50">
                      ...
                      <a href={connectUri} onClick={(e) => { e.preventDefault(); window.location.href = connectUri; }} className="flex items-center justify-center gap-2 text-xs text-lc-white hover:bg-lc-green/20 transition px-3 py-2 bg-lc-green/10 rounded-xl border border-lc-green/30">
                    ...
                    {waitingForScan && (
                      <div className="mt-2 flex flex-col items-center gap-2 text-lc-green text-sm">
                        <div className="flex items-center gap-2">
                          <div className="lc-spinner" style={{ width: 14, height: 14, borderWidth: '2px' }} />
                          Waiting for connection...
                        </div>
                        {showSlowHint && (
                          <p className="text-xs text-lc-muted text-center animate-in fade-in duration-500">
                            Still waiting? Switch back to Amber to make sure you approved.
                          </p>
                        )}
                      </div>
                    )}
                  ...
                  <div className="py-8 flex flex-col items-center gap-3">
                    <div className="lc-spinner" style={{ width: 24, height: 24, borderWidth: '2px' }} />
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-lc-muted mb-2 font-medium">Paste your bunker URL</label>
                  <input
                    type="text"
                    value={bunkerInput}
                ...
                <button onClick={() => handleLogin('bunker')} disabled={isLoading || !bunkerInput.trim()} className="w-full lc-pill lc-pill-primary text-sm disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2">
