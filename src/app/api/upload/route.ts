...
export async function POST(req: NextRequest) {
  ...
  // Default to a site-relative `/uploads/<name>` — this always resolves
  // against whatever hostname the browser loaded the app from, avoiding the
  // `https://0.0.0.0:3000` 404 that `req.nextUrl.origin` produces behind a
  // custom server. When Caddy (or any reverse proxy) sets `x-forwarded-proto`,
  // we upgrade to an absolute URL built from the forwarded Host + proto so
  // downstream consumers that need a fully-qualified URL still get one.
  const forwardedProto = req.headers.get('x-forwarded-proto');
  const forwardedHost = req.headers.get('host');
  const url = forwardedProto && forwardedHost
    ? `${forwardedProto}://${forwardedHost}/uploads/${storedName}`
    : `/uploads/${storedName}`;
