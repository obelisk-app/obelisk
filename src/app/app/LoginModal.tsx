...
/**
 * Obelisk's original La Crypta–styled login modal.
 *
 * Visual chrome is the obelisk-native panel (own backdrop, lc-pill
 * buttons, lc-spinner, lime-green accents). The signing pipeline is the
 * bridge — `nostrActions.loginWithNip07 / loginWithNsec / loginWithBunker`
 * + `nostrActions.createNostrConnectSession` for QR — so the SDK's
 * `useSignerStorage` doesn't end up duplicating the bridge's session.
 *
 * Why this lives outside `@nostr-wot/ui`'s LoginModal:
 *   - the SDK widget defaults to only 2 NIP-46 rendezvous relays
 *     (relay.nsec.app + relay.damus.io). When nsec.app is rate-limited
 *     the QR pairing silently times out; the bridge's NOSTRCONNECT_RELAYS
 *     set has more fallbacks (see client.ts).
 *   - the SDK widget creates a parallel signer cache that the bridge
 *     doesn't know about — leaks across logins on the same browser.
 *   - obelisk's UI matches the rest of the app's La Crypta design.
 */
...
type LoginMethodId = 'nip07' | 'import' | 'generate' | 'nip46';
type InternalMethod = 'extension' | 'nsec' | 'bunker';
type BunkerTab = 'qr' | 'url';

interface LoginModalProps {
  onSuccess?: () => void;
  /** When provided, restrict the modal to these methods. Mobile uses this
   *  to scope the popup to the single method the user tapped. */
  methods?: ReadonlyArray<LoginMethodId>;
  /** Caller-provided dismiss; without it, the modal can't be closed
   *  (matches the desktop AppShell case where login is mandatory). */
  onClose?: () => void;
  /** Override the modal's heading. */
  title?: string;
  /** Override the modal's subheading. */
  subtitle?: string;
}

const ID_TO_INTERNAL: Record<LoginMethodId, InternalMethod | 'generate'> = {
  nip07: 'extension',
  import: 'nsec',
  nip46: 'bunker',
  generate: 'generate',
...
function isMethodAllowed(
  candidate: LoginMethodId,
  methods?: ReadonlyArray<LoginMethodId>,
): boolean {
  return !methods || methods.includes(candidate);
}
...
export default function LoginModal({
  ...
  const showExtension = isMethodAllowed('nip07', methods);
  const showNsec = isMethodAllowed('import', methods);
  const showBunker = isMethodAllowed('nip46', methods);
  const showGenerate = isMethodAllowed('generate', methods);

  // When the modal is scoped to a single non-list method, jump directly
  // into that method's panel.
  const initialMethod: InternalMethod | null = (() => {
    if (!methods || methods.length !== 1) return null;
    const only = methods[0];
    if (only === 'import') return 'nsec';
    if (only === 'nip46') return 'bunker';
    return null;
  })();

  const [method, setMethod] = useState<InternalMethod | null>(initialMethod);
  const [nsecInput, setNsecInput] = useState('');
  const [bunkerInput, setBunkerInput] = useState('');
  const [bunkerTab, setBunkerTab] = useState<BunkerTab>('qr');
  const [loadingMethod, setLoadingMethod] = useState<InternalMethod | null>(null);
  ...
  // For the single-method "generate" entry point — auto-trigger so the
  // user lands directly on the backup screen.
  const autoGenerated = useRef(false);

  useEffect(() => {
    const check = () => setHasNip07(typeof window !== 'undefined' && !!(window as { nostr?: unknown }).nostr);
    check();
  ...
  const handleLogin = useCallback(async (loginMethod: InternalMethod) => {
    ...
    try {
      switch (loginMethod) {
        case 'extension': {
          ...
          if (!w.nostr) {
            throw new Error('Nostr extension not detected. Install Alby, nos2x, or similar.');
          }
  ...
  const handleCreateAccount = useCallback(async () => {
    ...
    try {
      ...
      try { window.localStorage.setItem(`obelisk-dex/just-generated/${pk}`, '1'); } catch { /* ignore */ }
    } catch (err) {
  ...
  // Auto-trigger account generation when the modal is scoped to "generate".
  useEffect(() => {
    if (autoGenerated.current) return;
    if (!methods || methods.length !== 1 || methods[0] !== 'generate') return;
  ...
  return (
    <div
      className="lc-grid-bg fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
        className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-lc-border bg-lc-dark p-6 shadow-2xl sm:p-8"
        onClick={(e) => e.stopPropagation()}
      >
        <div className={`mb-6 flex items-center ${newAccountNsec ? 'justify-end' : 'justify-between'}`}>
          {!newAccountNsec && (
            <div>
              <h2 className="text-xl font-bold text-lc-white">{title}</h2>
              <p className="mt-1 text-sm text-lc-muted">{subtitle}</p>
            </div>
          )}
          {onClose && (
            <button
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-lc-border bg-lc-black p-3.5 font-mono text-sm text-lc-white placeholder-lc-border transition focus:border-lc-green/50 focus:outline-none focus:ring-1 focus:ring-lc-green/20"
                  />
                  <p className="mt-2.5 text-xs text-lc-muted">Get this from your nsec.app, Amber, or other remote signer.</p>
                </div>
                <button
